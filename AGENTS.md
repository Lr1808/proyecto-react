...
- Quirk — the app is plain JS (`src/App.jsx`, `src/components/*.jsx`, `src/lib/*.js`) with a single TypeScript entry `src/main.tsx` that just mounts `App.jsx`; `index.html` points to `/src/main.tsx`. Match the style of the file you touch; do not add type annotations to a `.jsx` file.
