...
export default function StudentDashboard({ user, token, apiBase }) {
  ...
  return <div className="page-content">
    ...
    {activeTab === 'hybrid' ? <HybridTaskManager /> : activeTab === 'route' ? <RouteMap catalog={catalog} attempts={attempts} onSelect={(item) => setActiveExercise(item)} /> : <><div className="catalog-filters"><div className="filter-bar"><Filter size={15} />{statusFilters.map((value) => <button className={filter === value ? 'active' : ''} onClick={() => setFilter(value)} type="button" key={value}>{value}</button>)}</div><div className="filter-bar category-bar">{categoryOptions.map((value) => <button className={categoryFilter === value ? 'active' : ''} onClick={() => setCategoryFilter(value)} type="button" key={value}>{value}</button>)}</div></div><div className="exercise-grid">{visible.map((item) => { const attempt = attempts[item.id]; const done = Boolean(attempt); const statusLabel = done ? (attempt.passed ? `Aprobado ${attempt.best_percentage}%` : `Reprobado ${attempt.best_percentage}%`) : 'Pendiente'; const ctaLabel = item.is_exam ? (done ? 'Reintentar examen' : 'Iniciar examen') : (done ? 'Volver a practicar' : 'Comentar ahora'); return <article className="exercise-tile" key={item.id}><div className="tile-top"><span className="subject-pill">{item.is_exam ? 'EXAMEN · ' : ''}{item.subject}</span><span className={done ? 'done-status' : 'pending-status'}>{statusLabel}</span></div><h3>{item.title}</h3><p>{item.description}</p><div className="tile-meta"><span><Clock3 size={14} /> {item.time}</span><span><Target size={14} /> {item.points} pts</span><span className="level-tag">{item.level}</span>{attempt?.attempts > 1 && <span className="level-tag">{attempt.attempts} intentos</span>}</div><div className="tile-bottom">{item.programming_language === 'python' && <span className="judge-badge"><Code2 size={13} /> Escribe código</span>}<button type="button" className="tile-button" onClick={() => setActiveExercise(item)}><Play size={14} /> {ctaLabel}</button>{done && <button type="button" className="history-button" onClick={() => setHistoryExercise(item)}><History size={13} /> Ver intentos</button>}</div></article> })}</div></>}
